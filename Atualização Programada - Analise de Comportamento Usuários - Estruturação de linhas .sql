DELETE FROM `projeto.raw_marketing.at_di_dashboard_novofluxo` where dt_evento >= (current_date()-3)
;
insert into `projeto.raw_marketing.at_di_dashboard_novofluxo` 
Select * from (
--EVENTOS DO FLUXO ONDE ENCONTRAR
WITH Eventos_Fluxo as (
Select  
...
FROM
  `projeto.tabela.events_*` a
   where lower(event_name) like '%fluxo%'
and 'Onde encontrar' in (SELECT value.string_value FROM UNNEST(event_params) WHERE key = 'page_title') 
...
order by 1 asc, 3 asc
  ...
  Select a.*, b.qtd_formulario from Eventos_Fluxo a left join (Select *,ROW_NUMBER() OVER (PARTITION BY  id_sessao ORDER BY min(dt_evento)) as qtd_formulario,
...
BOTAO_ONDE_COMPRAR_TRATADO as (
  with BOTAO_ONDE_COMPRAR as (
  Select distinct
  datetime(timestamp_micros(event_timestamp), 'America/Sao_Paulo') as dt_evento,
  (SELECT value.int_value FROM UNNEST(event_params) AS event_param  WHERE event_param.key = 'ga_session_id') as id_sessao,
  ...
  FROM
    `projeto.tabela.events_*` 
    where regexp_contains(lower(event_name),'clique_donde_encontrar|clique_onde_comprar') and event_name not like 'clique_onde_comprar_instalar'
    and datetime(timestamp_micros(event_timestamp), 'America/Sao_Paulo') > '2024-03-04'
  order by id_sessao asc, dt_evento asc
  )
  select 
